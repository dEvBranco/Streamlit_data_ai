...
def main():
    ...
    if task == "Load from local drive, <200MB":
        ...
        if col_desc == "Yes":
            addon = st.text_input(
                "Enter your column description, e.g. 'col1': 'unique id'"
            )
        else:
            addon = "None"
        ...
        if addon:
            ...
            if llm == "PandasAI":
                connector = PandasConnector(
                    {"original_df": df}, field_descriptions=addon
                )
    ...
    if task == "Load from local drive, 200MB+":
        ...
        if addon:
            llm = OpenAI(api_token=OPENAI_API_KEY)
            connector = PandasConnector(
                {"original_df": df_large}, field_descriptions=addon
            )
            sdf = SmartDataframe(
                connector,
                {"enable_cache": False},
    ...
    if task == "Load from Google Storage":
        ...
        if addon:
            llm = OpenAI(api_token=OPENAI_API_KEY)
            connector = PandasConnector(
                {"original_df": df_bq}, field_descriptions=addon
            )
            sdf = SmartDataframe(
                connector,
                {"enable_cache": False},
    ...
    if task == "Yahoo Finance":
        ...
        yahoo_df = SmartDataframe(
            yahoo_connector, config={"response_parser": OutputParser}
        )
